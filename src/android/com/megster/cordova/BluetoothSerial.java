...
public class BluetoothSerial extends CordovaPlugin {
    ...
    private static final HashMap<Integer, String> CONNECTION_STATES = new HashMap<Integer, String>() {
        {
            this.put(BluetoothProfile.STATE_CONNECTED, "BLUETOOTH CONNECTED");
            this.put(BluetoothProfile.STATE_CONNECTING, "BLUETOOTH CONNECTING");
            this.put(BluetoothProfile.STATE_DISCONNECTED, "BLUETOOTH DISCONNECTED");
            this.put(BluetoothProfile.STATE_DISCONNECTING, "BLUETOOTH DISCONNECTING");
            this.put(STATE_UNKNOWN, "BLUETOOTH STATUS UNKNOWN");
        }
    } ;
    ...
    private void listBondedDevices(CallbackContext callbackContext) {
        ...
        */
        ...
        //setup proxy to monitor headset profile connections
        bluetoothAdapter.getProfileProxy(cordova.getActivity(), new BluetoothProfile.ServiceListener() {
            @Override
            public void onServiceConnected(int profile, BluetoothProfile proxy) {
                Log.i("ServiceConnected", Integer.toString(profile));
                JSONArray deviceList = new JSONArray();
                //check if headset is already connected before our app started
                int headsetConnectionState = bluetoothAdapter.getProfileConnectionState(BluetoothProfile.HEADSET);
                if (headsetConnectionState == BluetoothProfile.STATE_CONNECTED) {
                    //remember connected device
                    List<BluetoothDevice> connectedDevices = proxy.getConnectedDevices();
                    if (connectedDevices.size() > 0) {
                       //  mConnectedDevice = proxy.getConnectedDevices().get(0);
                        //  displayTextAndTimeStamp(CONNECTION_STATES.get(BluetoothProfile.STATE_CONNECTED));
            ...
            public void onServiceDisconnected(int profile) {
                Log.i("ServiceDisconnected", Integer.toString(profile));
            }
        ...
        final BroadcastReceiver mBroadCastReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (action.equals(BluetoothHeadset.ACTION_CONNECTION_STATE_CHANGED)) {
                    ...
                    if (device != null) {
                        Log.i(TAG, String.format("%s %s %d",device.getName(), device.getAddress(), device.getBluetoothClass().getDeviceClass()));

                        int previousState = intent.getIntExtra(BluetoothProfile.EXTRA_PREVIOUS_STATE, STATE_UNKNOWN);
                        int currentState = intent.getIntExtra(BluetoothProfile.EXTRA_STATE, STATE_UNKNOWN);
                        String previousStateStr = CONNECTION_STATES.get(previousState);
                        String currentStateStr = CONNECTION_STATES.get(currentState);

                        if (CONNECTION_STATES.containsKey(previousState)) {
                            Log.i(TAG, previousStateStr);
                        }
                        if (CONNECTION_STATES.containsKey(currentState)) {
                            Log.i(TAG, currentStateStr);
                        }

                        // displayTextAndTimeStamp(currentStateStr);
                        try {
                        ...
                        //remember connected device
                        if (previousState == BluetoothProfile.STATE_CONNECTING && currentState == BluetoothProfile.STATE_CONNECTED) {
